...
1. **Base Validations (Regression & New)** **Given** an invalid request (e.g., missing `modelId`, negative tokens) **When** backend validates request **Then** return HTTP 400 with appropriate error messages. 
2. **Standard Plan with Model-Aware Overage** **Given** a “Standard” customer with a 100,000 monthly quota, 90,000 used so far. Overage for “fast-model” is `$0.01/1K`. **When** submitting 30,000 tokens for “fast-model” **Then** bill shows: 10,000 from quota, 20,000 overage, $0.20 charge. 
3. **Premium Plan with Split Rates** **Given** a “Premium” customer. For “reasoning-model”, Prompt is `$0.03/1K`, Completion is `$0.06/1K`. **When** submitting 10,000 prompt and 20,000 completion tokens for “reasoning-model” **Then** bill shows: 0 from quota, `$0.30` prompt charge, `$1.20` completion charge, total `$1.50`. 

1. 1. **Flexible Monetization**: Support diverse billing strategies (Standard, Premium) to capture different market segments.
2. 2. **Model-Aware Pricing**: Charge different rates based on the specific AI model used.
3. 3. **Architecture Scalability**: Implement an extensible design (e.g., Strategy Pattern) to isolate calculation logic and easily add future pricing models.
- • Enhance the existing POST /api/usage endpoint.
- • **New Request Field:** Add modelId (required, string, e.g., “fast-model”, “reasoning-model”).
- • Implement a routing mechanism (Strategy/Factory Pattern) to handle distinct calculation formulas.
- • Implement two initial Plan Types: 
    1. 1. **Standard Plan (Legacy Refactor):** Has a monthly global quota. Overage rates now depend on the modelId.
    2. 2. **Premium Plan (New):** No quota. Prompt and Completion tokens are billed separately, and rates vary by modelId.
4. 
5. 
- • Complex tiered/volume-based discount logic (Deferred to Phase 2).
- • Subscription plan creation and assignment CRUD.
- • Invoice generation.
6. 1. **Base Validations (Regression & New)** **Given** an invalid request (e.g., missing `modelId`, negative tokens) **When** backend validates request **Then** return HTTP 400 with appropriate error messages.
7. 2. **Standard Plan with Model-Aware Overage** **Given** a “Standard” customer with a 100,000 monthly quota, 90,000 used so far. Overage for “fast-model” is `$0.01/1K`. **When** submitting 30,000 tokens for “fast-model” **Then** bill shows: 10,000 from quota, 20,000 overage, $0.20 charge.
8. 3. **Premium Plan with Split Rates** **Given** a “Premium” customer. For “reasoning-model”, Prompt is `$0.03/1K`, Completion is `$0.06/1K`. **When** submitting 10,000 prompt and 20,000 completion tokens for “reasoning-model” **Then** bill shows: 0 from quota, `$0.30` prompt charge, `$1.20` completion charge, total `$1.50`.
...
We will see a concrete, step-by-step example of this exact “prompt-first update” workflow later in [where we modify the prompt](https://martinfowler.com/articles/structured-prompt-driven/#logic-correction) to correct a business logic mismatch before touching any code.
...
9. Architecture: does the code strictly follow our expected 3-tier architecture?
10. Business logic: does the Service layer implementation perfectly align with our initial intent?
11. Scope of change: are the modifications strictly confined to the boundaries defined by the structured prompt, avoiding unrelated changes or scope creep?
...
12. Identify the mismatch: notice that the behavior of `modelId` during persistence is inconsistent with the new business requirement (it must be mandatory with a default).
13. Target the prompt snippet: copy the specific section from the structured prompt that defines the outdated logic.
14. Update the prompt: paste the extracted snippet into the chat alongside the revised business rule, instructing the AI to update the structured prompt first.
15. Generate targeted code updates: once the prompt reflects the new truth, run `/spdd-generate` pointing to the updated file. The AI automatically performs targeted diffs exclusively on the affected codebase, rather than regenerating everything from scratch.
...
16. A business logic implementation with exceptionally high intent alignment (~99%).
17. Complete engineering transparency, including a clear understanding of the implementation path, technical decisions, and accepted trade-offs.
18. A structured prompt asset tightly synchronized with the current codebase, laying a solid foundation for future iterations.
19. Compounding human expertise, fostering a continuous accumulation of developer experience and mental models as we iterate collaboratively with the AI.
